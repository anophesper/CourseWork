...
{
    public partial class WorkersForm : BaseForm
    {
        private List<User> admins = Program.DataManager.Users.Where(u => u is BranchAdmin || u is SystemAdmin).ToList();
        ...
        private void SetupDataGridView()
        {
            ...
            var dataSource = admins.Select(u => new
            {
                ...
                Role = u is BranchAdmin ? "Адміністратор відділення" : "Адміністратор системи",
                BranchInfo = (u as BranchAdmin)?.Branch != null
                    ? $"{(u as BranchAdmin).Branch.Id}, {(u as BranchAdmin).Branch.City}, {(u as BranchAdmin).Branch.Address}"
                    : ""
            }).ToList();
        ...
        private void SaveNewUser(DataGridViewRow row)
        {
            ...
            if (role == "Адміністратор відділення")
            {
                ...
                {
                    BranchAdmin newAdmin = new BranchAdmin(firstName, lastName, phoneNumber, password, branch);
                    DB_DataManager.InsertIntoDatabase(newAdmin);
                }
                else if (role == "Адміністратор системи")
                {
                    SystemAdmin newAdmin = new SystemAdmin(firstName, lastName, phoneNumber, password);
                    DB_DataManager.InsertIntoDatabase(newAdmin);
                }
            }
        }
